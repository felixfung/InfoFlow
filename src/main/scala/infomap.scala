...
object InfoMap
{
  ...
  (Int,Int,Double,Double,Double,Double,Double,Double,Double,Double,Double) = {
    ...
    if( q12 > 0 ) {
      val (delta_q12,deltaL12) = Partition.calDeltaL(qi_sum,q1,q2,q12,p1,p2)
      (n1,n2,p1,p2,w1,w2,w12,q1,q2,deltaL12,delta_q12)
    }
    else { // q12==0 iff we are merging the entire graph into one module
      (n1,n2,p1,p2,w1,w2,w12,q1,q2,0.0,0.0)
    }
  }
}
...
class InfoMap( outputDir: String ) extends MergeAlgo(outputDir)
{
   ...
   ***************************************************************************/
      ...
      codeLength: Double ) = {
        ...
        logFile.close
        val iWj = table.map {
          case ((from,to),(_,_,_,_,_,_,w12,_,_,_,_))
          => ((from,to),w12)
        }
   ...
   * or if the code length cannot be decreased,
   * then terminate;
   ...
   ***************************************************************************/
      if( table.count == 0 )
      {
        ...
        val( (merge1,merge2), (n1,n2,p1,p2,w1,w2,w102,q1,q2,deltaL12,delta_q12) )
          = table.reduce {
          case (
            (
              (merge1A,merge2A),
              (n1A,n2A,p1A,p2A,w1A,w2A,w12A,q1A,q2A,deltaL12A,delta_q12A)
            ),
            (
              (merge1B,merge2B),
              (n1B,n2B,p1B,p2B,w1B,w2B,w12B,q1B,q2B,deltaL12B,delta_q12B)
            )
          )
          => if( Partition.calDeltaL( deltaL12A, qi_sum, delta_q12A )
            < Partition.calDeltaL( deltaL12B, qi_sum, delta_q12B ) )
               ((merge1A,merge2A),
                 (n1A,n2A,p1A,p2A,w1A,w2A,w12A,q1A,q2A,deltaL12A,delta_q12A))
             else
               ((merge1B,merge2B),
        ...
        val deltaL = Partition.calDeltaL( deltaL12, qi_sum, delta_q12 )
   ...
   ***************************************************************************/
          ...
          .map {
            ...
            case ((from,to),(ni,nj,pi,pj,wi,wj,wij,qi,qj,deltaLij,delta_qij)) =>
              if( from==merge1 || from==merge2 )
                ((merge1,to),(n12,nj,p12,pj,w12,wj,wij,q12,qj,0.0,0.0))
              else if( to==merge1 )
                  ((from,merge1),(ni,n12,pi,p12,wi,w12,wij,qi,q12,0.0,0.0))
              else if( to==merge2 ) {
                if( merge1 < from )
                  ((merge1,from),(n12,ni,p12,pi,w12,wi,wij,q12,qi,0.0,0.0))
                else
                  ((from,merge1),(ni,n12,pi,p12,wi,w12,wij,qi,q12,0.0,0.0))
              }
              else
                ((from,to),(ni,nj,pi,pj,wi,wj,wij,qi,qj,deltaLij,delta_qij))
          }
          .reduceByKey { // aggregate inter-modular connection weights
            case (
              (ni,nj,pi,pj,wi,wj,wij,qi,qj,deltaLij,delta_qij),
              (_,_,_,_,_,_,wji,_,_,_,_)
            )
            => (ni,nj,pi,pj,wi,wj,wij+wji,qi,qj,deltaLij,delta_qij)
          }
          .map {
            case (
              (from,to),
